...
munit_case(RUN, test_update_file_basic, {
    ...
    munit_assert_int(filedata[1], ==, 100, "Number of lines should be update to 100");
    munit_assert_int(filedata[2], ==, 1, "File type should be update to 1");
...
munit_case(SUBMIT, test_update_file_different_values, {
    ...
    munit_assert_int(filedata[1], ==, 250, "Number of lines should be update to 250");
    munit_assert_int(filedata[2], ==, 3, "File type should be update to 3");
    munit_assert_int(filedata[199], ==, 0, "Last element should be set to 0");
});
