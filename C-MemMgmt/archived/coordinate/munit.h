...
// munit_assert_int macro
#define munit_assert_int(a, op, b, message) do { \
    if (!((a) op (b))) { \
...
// munit_assert_ptr_not_equal macro
#define munit_assert_ptr_not_equal(a, b, message) do { \
    if ((a) == (b)) { \
        printf("\nAssertion Failed in test '%s': %s\n", __func__, message); \
        printf("  Expected pointers to be different, but they are equal: %p\n", (void*)(a)); \
        return "Pointer equality failure"; \
    } \
} while(0)
